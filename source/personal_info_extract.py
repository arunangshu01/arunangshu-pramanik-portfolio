...
from model.base_models_portfolio import PersonalInformation, Location, SocialMediaProfiles, ErrorResponseModel
from utility.settings import logger, personal_info_data
...
class PersonalInformationGenerator:
    ...
    def _format_and_validate_personal_info_data(self):
        personal_info_data_df = pd.json_normalize(
            ...
            meta=[
                'name', "phone", ["location", "city"], ["location", "state"],
                ["location", "country"], "email",
                ["social_media_profiles", "linkedin"], ["social_media_profiles", "github"],
    ...
    def display_personal_info(self):
        try:
            personal_info = self._personal_info_data()
            return personal_info
